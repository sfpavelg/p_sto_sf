...
1. Тесты создаются согласно REST-контроллерам, в начале названия добавляется Test
```Например для контроллера ResourseAnswerController создается тест TestResourseAnswerController, в котором тестируются все API этого контроллера```
2. В пакетах dataset/expected лежат все датасеты для сравнения результатов теста. Данные датесеты не используются для обычной загрузки данных для тестов и используются только для операции DELETE, UPDATE, PERSIST
3. Все сущности, описанные в датасетах для загрузки тестовых данных, начинаются с id = 100
4. Все тестовые классы должны наследоваться от абстрактного класса, где описана все конфигурация тестов
5. НЕЛЬЗЯ ИЗМЕНЯТЬ УЖЕ НАПИСАННЫЕ ДАТАСЕТЫ! Для того, чтобы добавить датасет, необходимо создать подпакет для своего конкретного теста и в нем создать необходимые датасеты
6. На каждый класс тестов должен быть написан отдельный датасет. При необохдимости внесения изменений в датасеты применяется п. 5.
```Например если мы тестируем ResourseAnswerController, есть подпакет dataset/[название пакета] в котором хранятся все датасеты для тестирования этого контроллера```
7. Нельзя использовать аннотации тразакции для тестов
8. Нельзя ставить аннотацию DataSet над классом. Для каждого отдельного метода теста создан отдельный датасет
9. Все классы для тестов находиться в подпакете test/java/.../api